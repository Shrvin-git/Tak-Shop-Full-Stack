import styles from "./BlogNav.module.css";

function BlogNav() {
  return (
    <section className={styles["blog-header"]}>
      <div className={styles["blog-header-righ"]}>
        <div className={styles["blog1"]}>
          <img src="./images/blog-imgs/1png.png" alt="" />
        </div>

        <div className={styles["blog2"]}>
          <img src="./images/blog-imgs/6.jpg" alt="" />

          <div className={styles["blog-desc"]}>
            <div className={styles["blog-desc-wrapper"]}>
              <h2 className={styles["blog-desc-title"]}>
                بهترین لپ تاب های گیمینگ سال 2025
              </h2>

              <div className={styles["blog-info"]}>
                <div className={styles["blog-info-date"]}>
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M8 14C7.71667 14 7.47933 13.904 7.288 13.712C7.09667 13.52 7.00067 13.2827 7 13C6.99933 12.7173 7.09533 12.48 7.288 12.288C7.48067 12.096 7.718 12 8 12C8.282 12 8.51967 12.096 8.713 12.288C8.90633 12.48 9.002 12.7173 9 13C8.998 13.2827 8.902 13.5203 8.712 13.713C8.522 13.9057 8.28467 14.0013 8 14ZM12 14C11.7167 14 11.4793 13.904 11.288 13.712C11.0967 13.52 11.0007 13.2827 11 13C10.9993 12.7173 11.0953 12.48 11.288 12.288C11.4807 12.096 11.718 12 12 12C12.282 12 12.5197 12.096 12.713 12.288C12.9063 12.48 13.002 12.7173 13 13C12.998 13.2827 12.902 13.5203 12.712 13.713C12.522 13.9057 12.2847 14.0013 12 14ZM16 14C15.7167 14 15.4793 13.904 15.288 13.712C15.0967 13.52 15.0007 13.2827 15 13C14.9993 12.7173 15.0953 12.48 15.288 12.288C15.4807 12.096 15.718 12 16 12C16.282 12 16.5197 12.096 16.713 12.288C16.9063 12.48 17.002 12.7173 17 13C16.998 13.2827 16.902 13.5203 16.712 13.713C16.522 13.9057 16.2847 14.0013 16 14ZM5 22C4.45 22 3.97933 21.8043 3.588 21.413C3.19667 21.0217 3.00067 20.5507 3 20V6C3 5.45 3.196 4.97934 3.588 4.588C3.98 4.19667 4.45067 4.00067 5 4H6V3C6 2.71667 6.096 2.47934 6.288 2.288C6.48 2.09667 6.71733 2.00067 7 2C7.28267 1.99934 7.52033 2.09534 7.713 2.288C7.90567 2.48067 8.00133 2.718 8 3V4H16V3C16 2.71667 16.096 2.47934 16.288 2.288C16.48 2.09667 16.7173 2.00067 17 2C17.2827 1.99934 17.5203 2.09534 17.713 2.288C17.9057 2.48067 18.0013 2.718 18 3V4H19C19.55 4 20.021 4.196 20.413 4.588C20.805 4.98 21.0007 5.45067 21 6V20C21 20.55 20.8043 21.021 20.413 21.413C20.0217 21.805 19.5507 22.0007 19 22H5ZM5 20H19V10H5V20ZM5 8H19V6H5V8Z"
                      fill="#2A2A2A"
                    ></path>
                  </svg>
                  9 بهمن
                </div>

                <div className={styles["blog-info-category"]}>
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <g clipPath="url(#clip0_1058_5485)">
                      <path
                        d="M4.50943e-06 3.75C4.50943e-06 3.15326 
                        ... FULL PATH REMAINS ...
                        "
                        fill="#2A2A2A"
                      ></path>
                      <path
                        d="
                        M3 4.5H18V7.5H3V4.5ZM3 9H9V13.5H3V9ZM3 15H9V16.5H3V15ZM3 18H9V19.5H3V18Z
                        "
                        fill="#2A2A2A"
                      ></path>
                    </g>
                    <defs>
                      <clipPath id="clip0_1058_5485">
                        <rect width="24" height="24" fill="white"></rect>
                      </clipPath>
                    </defs>
                  </svg>
                  پیشنهادات
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* LEFT SIDE */}
      <div className={styles["blog-header-left"]}>
        <div className={styles["blog3"]}>
          <img src="./images/blog-imgs/2.png" alt="" />

          <div className={styles["blog-desc"]}>
            <div className={styles["blog-desc-wrapper"]}>
              <h2 className={styles["blog-desc-title"]}>
                معرفی و بررسی گوشی سامسونگ S25 Ultra
              </h2>

              <div className={styles["blog-info"]}>
                <div className={styles["blog-info-date"]}>
                  {/* SVG کامل و بدون تغییر */}
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M8 14C7.71667 14 7.47933 13.904 ...
                      "
                      fill="#2A2A2A"
                    ></path>
                  </svg>
                  9 بهمن
                </div>

                <div className={styles["blog-info-category"]}>
                  {/* SVG کامل */}
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <g clipPath="url(#clip0_1058_5485)">
                      <path
                        d="
                        M4.50943e-06 3.75...
                        "
                        fill="#2A2A2A"
                      ></path>

                      <path
                        d="
                        M3 4.5H18V7.5H3V4.5...
                        "
                        fill="#2A2A2A"
                      ></path>
                    </g>
                    <defs>
                      <clipPath id="clip0_1058_5485">
                        <rect width="24" height="24" fill="white"></rect>
                      </clipPath>
                    </defs>
                  </svg>
                  پیشنهادات
                </div>
              </div>
            </div>
          </div>
        </div>

        {/* BLOG 4 */}
        <div className={styles["blog4"]}>
          <img src="./images/blog-imgs/3.png" alt="" />

          <div className={styles["blog-desc"]}>
            <div className={styles["blog-desc-wrapper"]}>
              <h2 className={styles["blog-desc-title"]}>
                کارت گرافیک RTX 5090 معرفی شد!
              </h2>

              <div className={styles["blog-info"]}>
                <div className={styles["blog-info-date"]}>
                  {/* SVG کامل */}
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M8 14C7.716..." fill="#2A2A2A"></path>
                  </svg>
                  9 بهمن
                </div>

                <div className={styles["blog-info-category"]}>
                  {/* SVG کامل */}
                  <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <g clipPath="url(#clip0_1058_5485)">
                      <path d="M4.5094..." fill="#2A2A2A"></path>
                      <path d="M3 4.5..." fill="#2A2A2A"></path>
                    </g>
                    <defs>
                      <clipPath id="clip0_1058_5485">
                        <rect width="24" height="24" fill="white"></rect>
                      </clipPath>
                    </defs>
                  </svg>
                  نقد و بررسی
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

export default BlogNav;
